...
person item and location */
        ...
        this.searchchat = function (txt, qs, del) {
            ...
            if (chat[qs]) {
                ...
                for (x in chatstage.always) {
                    if (x == txt) {
                        if (del) {
                            chatstage.always[x] = [chat.invalid];
                            return true;
                        }
    ...
    function Person(name, desc, chat) {
        ...
        p.use = function () {
            writeOut("Using people is wrong.");
            return false;
        };
        ...
        p.kill = function () {
            ...
            this.talk = function () {
                writeOut("You cannot talk to dead people.");
                return false;
            };
        };
        return p;
    ...
    function Item(name, desc) {
        ...
        i.get = function () {
            if (this.getable) {
                if (this.held) {
                    writeOut("You already have that in your inventory, where else are you going to put it?");
                    return false;
                }
    ...
    function load() {
        ...
        for (i = 0; i < cookies.length; i++) {
            if (cookies[i].match(/420ta=/)) {
                loadnum = cookies[i].replace(/420ta=/, "");
                break;
            }
        }
        ...
        case 2:
            ...
            break;
        default:
            writeOut("There is no file to load from. This game only saves at certain checkpoints.");
            break;
        }
    }
    ...
    function redditdeath() {
        ...
        philtardNPC.kill();
        avgredditorNPC.kill();
        redditAREA.desc = reddit.altdesc;
        memesAREA.desc = memes.altdesc;
        todayilearnedAREA.desc = todayilearned.altdesc;
        catsAREA.desc = cats.altdesc;
    ...
    function textIn(ui) {
        ...
        case ui == "load":
            load();
            break;
        case looklist.test(ui):
            look(ui.replace(looklist, ""));
            break;
        case getlist.test(ui):
            get(ui.replace(getlist, ""));
            break;
        case talklist.test(ui):
            talk(ui.replace(talklist, ""));
            break;
        case invlist.test(ui):
            writeOut("Your inventory contains:<br/><br/>" + inv.list());
            break;
        case uselist.test(ui):
            use(ui.replace(uselist, ""));
            break;
        case movelist.test(ui):
            move(ui.replace(movelist, ""));
            break;
        case droplist.test(ui):
            drop(ui.replace(droplist, ""));
            break;
        case writelist.test(ui):
            writepost(ui.replace(writelist, ""));
            break;
        case ui == "perogi is great":
            inv.push(naziflagITEM);
            inv.push(feministuniformITEM);
            shesaidarchiveAREA.discovered = true;
            break;
        default:
            writeOut(invalidverb.pick());
            break;
        }
        return true;
    }
    ...
    function get(txt) {
        if (!txt) {
            writeOut("You can't pick up nothing, faggot.");
            return false;
        }
        var itemarr = currentloc.contain.concat(inv);
        if (o = itemarr.contains(txt)) {
    ...
    function talk(txt) {
        if (!txt) {
            writeOut("You talk to yourself for a while. It was a riveting conversation.");
            return 0;
        }
        var itemarr = currentloc.contain.concat(inv);
        if (o = itemarr.contains(txt)) {
    ...
    function use(txt) {
        if (!txt) {
            writeOut("You have to pick something to use");
            return false;
        }
        var itemarr = currentloc.contain.concat(inv);
        var items = [];
        for (i = 0; i < itemarr.length; i++) {
            if (txt.match(itemarr[i].name.toLowerCase())) {
                items.push(itemarr[i]);
            }
        }
        if (items.length == 2) {
            return useon(txt, items);
        }
        if (items.length == 1 && items[0].name.toLowerCase() == txt) {
            return items[0].use();
        }
    ...
    function drop(txt) {
        if (!txt) {
            writeOut("You drop nothing. Now no one can say you are clumsy.");
            return false;
        }
        if (o = inv.contains(txt)) {
            return o.drop();
        }
    ...
    function move(txt) {
        if (!txt) {
            writeOut("You shuffle around aimlessly a bit.");
            return false;
        }
    ...
    function writepost(txt) {
        switch (true) {
        case txt == "reddit" && redditAREA.discovered:
            writeOut("You compose an eloquent post about the failings of Reddit");
            inv.push(redditpostITEM);
            break;
        default:
            writeOut("You don't want to write about that");
            break;
        }
        return true;
    }
    ...
    howlieNPC.useon = function (user) {
        if (user == lighterITEM) {
            writeOut("Howlie says: Oh that's my lighter! I must have dropped it at Reddit. Uh, you can keep it :) as thanks for saving me. You'll probably find a use for it too!");
            return true;
        }
        writeOut("Howlie does not want that.");
        return false;
    };
    ...
    oldmanNPC.useon = function (user) {
        ...
        if (user == catflairITEM) {
            writeOut("Old Man says: hrgh, the hell you doing with that? Posting something in the cat subreddit isn't going to do anything, get a better flair.");
            return true;
        }
        writeOut(this.name + " does not want that");
        return false;
    ...
    dildoITEM.use = function () {
        writeOut("I'm sure you'd like to do that, but you're not allowed");
        return true;
    };
    var jointITEM = Item("joint", joint.desc);
    jointITEM.use = function () {
        writeOut("You smoke just a bit of the joint before putting it out. You get the feeling you might need it later.");
        return true;
    };
    var catpictureITEM = Item("cat picture", catpicture.desc);
    catpictureITEM.use = function () {
        writeOut(catpicture.desc + " Maybe someone would want this item?");
        return true;
    };
    ...
    naziflagITEM.get = function () {
        if (this.getable) {
            ...
            writeOut("You try to get the flag but Sachson puts a hand on your shoulder and shakes his head at you.");
            return false;
        }
    };
...
var toast = {
    chat: {
        ...
        0: {
            ...
            only: {
                cats: ["Toast shuffles through the pile of cat pictures and holds up a picture. On the picture is a link to a subreddit about cats<br/><strong>You received a link! You can now travel to the Cats subreddit.</strong>",
                    function () {
                        catsAREA.discovered = true;
                        return "delthis";
                    }
                ]
            }
    ...
    chat: {
        ...
        0: {
            ...
            only: {
                ...
                kidnapped: ["Everyone else has been kidnapped? O gawd they must have tricked me! Mmmmkay imma go back to 420streams then, meet yah back there!<br/><br/><strong>You have reached a checkpoint!</strong>",
                    function () {
                        streamAREA.contain.push(howlieNPC);
        ...
        1: {
            ...
            only: {
                ...
                "nazi forum": ["It's called Reich Forum I think. He left the link around here somewhere I think, oh yeah here it is!<br/><strong>You have recieved a link! You can now travel to the Reich Forums</strong>",
                    function () {
                        reichAREA.discovered = true;
                        return "delthis";
                    }
                ]
...
avgredditor = {
    chat: {
        ...
        0: {
            ...
            only: {
                subreddits: ["A subreddit is like, about a specific topic. My favorite is todayilearned. It has like, cool stories about people learning in their daily lives, you know? Everybody should post there and like, learn stuff everyday, man. Here, I'll give you the link.<br/><strong>You received a link! You can now travel to the todayilearned subreddit.</strong>",
                    function () {
                        todayilearnedAREA.discovered = true;
                        return "delthis";
                    }
                ],
                jokes: ["Yeah really funny, witty jokes, man. We call them 'memes' and they're really cool. Let me give you the link to the memes subreddit, dude. You'll thank me.<br/><strong>You received a link! You can now travel to the memes subreddit.</strong>",
                    function () {
                        memesAREA.discovered = true;
                        return "delthis";
                    }
                ]
            }
...
var lilithium = {
    ...
    chat: {
        ...
        1: {
            ...
            only: {
                favor: ["Hmmm well I suppose you could help. Just don't fuck this up okay? Take this message I wrote and post it on the Reich Forums, that'll show those fascists.<br/><strong>You have recieved an item!</strong>",
                    function () {
                        inv.push(feministpostITEM);
                        return "delthis";
                    }
                ]
            }
